...
class FTCLNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        # (16, 25, 2048) -> (16, 25, 1024)
        self.feature_trans = nn.Linear(in_features=self.feature_dim, out_features=self.fc_dim)

    def forward_once(self, input_features):
        ...
        # act_inst_cls_topk = torch.topk(act_inst_cas, k=inst_topk_num, dim=1)
        ...
        lcs_candi_idx, _ = torch.sort(sorted_act_inst_att_idx[:, : self.lcs_len, :], dim=1)
        lcs_candi = torch.gather(embeded_feature_rev, 1, lcs_candi_idx.expand([-1, -1, self.feature_dim]))

        fsd_act_candi_idx, _ = torch.sort(sorted_act_inst_att_idx[:, : self.fsd_len, :], dim=1)
        fsd_act_candi = torch.gather(embeded_feature_rev, 1, fsd_act_candi_idx.expand([-1, -1, self.feature_dim]))

        fsd_bak_candi_idx, _ = torch.sort(sorted_act_inst_att_idx[:, self.fsd_len: 2 * self.fsd_len, :], dim=1)
        fsd_bak_candi = torch.gather(embeded_feature_rev, 1, fsd_bak_candi_idx.expand([-1, -1, self.feature_dim]))

        fsd_act_candi = self.feature_trans(fsd_act_candi)
        fsd_bak_candi = self.feature_trans(fsd_bak_candi)
    ...
    def forward(self, ftcl, input_1, input_2=None):
        ...
        if not ftcl:
            ...
            return self.forward_once(input_1), self.forward_once(input_2)
